...
An inductor and a capacitor wired together form a tank, an LC resonator that sloshes energy back and forth between the magnetic field of the coil and the electric field of the cap. At one specific frequency, called the resonant frequency $f_0$, the two reactances cancel exactly and the tank looks pure-resistive from the outside. That cancellation is the whole trick behind powering a load through a small coil. A coil on its own is mostly an inductor, and at any useful operating frequency an AC source sees that as a large reactive impedance: almost all of the source's energy bounces back instead of crossing into the magnetic field, and the small fraction that does goes mostly into heating the wire rather than reaching a load. Add a series capacitor sized so $X_C = X_L$ at the operating frequency, and the two reactances cancel; what's left in the loop is only the small ohmic loss of the wire, so the source can finally push real current through, the coil's field builds up, and a second coil tuned to the same frequency on the receiver side gets enough flux through it to deliver power to its load. Both sides have to be tuned to the same $f_0$, which is the arithmetic the calculator handles: give it any two of $f$, $L$, $C$, plus an optional series resistance for quality factor, and the rest falls out.
...
The two schematics show the two ways to wire the tank. In a series LC the cancellation produces a short at $f_0$, which is what you want when feeding power from a voltage source: at resonance the loop impedance collapses to the parasitic resistance and current flows freely. In a parallel LC the cancellation produces an open at $f_0$, which is what you want when feeding from a current source or when you need to isolate one frequency from a wideband node. Either way, the quantity $Z_0 = \sqrt{L/C}$ is the tank's characteristic impedance, which is what you match the source against to maximize delivered power and the number a matching network has to land you on.

The losses in real components add up to an equivalent series resistance $R$: the coil's DC resistance plus AC skin and proximity effects, the capacitor's ESR, and, in a wireless-power link, the resistance reflected back from whatever the receiver-side coil is feeding. That total $R$ sets the tank's quality factor and the bandwidth around $f_0$:

$$Q = \frac{\omega_0 L}{R} = \frac{1}{R}\sqrt{\frac{L}{C}}, \qquad \Delta f_{-3\text{dB}} = \frac{f_0}{Q}$$

High Q means low loss and narrow bandwidth, which is the central trade-off of every wireless-power link: a high-Q coil delivers a larger fraction of the source's energy to the load, but a stray finger or a one-percent shift in component value can detune it right out of band; a low-Q coil tolerates sloppy alignment and component drift but burns more of the source's energy as heat in the windings.
